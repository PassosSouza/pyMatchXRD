...
def xyzToabc(xyz, L, Gam):
    ...
    A = np.array([
        ...
        [0, 0, c*np.sqrt(1 - np.cos(alpha)**2 - ( (np.cos(beta) - np.cos(alpha) * np.cos(gamma)) / np.sin(gamma) )**2 )]
    ])
...
def a_b_c_(L,Gamm):
    ...
    V = abs( np.dot( abcToxyz([1,0,0],L,Gamm) , np.cross(abcToxyz([0,1,0],L,Gamm) ,abcToxyz([0,0,1],L,Gamm) ) )   )
    V2 = abs( np.dot( abcToxyz([0,1,0],L,Gamm) , np.cross(abcToxyz([0,0,1],L,Gamm) ,abcToxyz([1,0,0],L,Gamm) ) )   )
    V3 = abs( np.dot( abcToxyz([0,0,1],L,Gamm) , np.cross(abcToxyz([1,0,0],L,Gamm) ,abcToxyz([0,1,0],L,Gamm) ) )   )
...
def INFOS_COD(name):
    ...
    return unit_param,unit_angles , Pos_atoms , syme , ATOMOS#,Occup
    

def generate_hkls(lamb, max_index, unit_params, unit_angles):
    """
    Gera os índices (hkl) possíveis e calcula os ângulos de difração.

    Parâmetros:
    lamb: Comprimento de onda da radiação.
    max_index: Índice máximo para h, k, l.
    unit_params: Parâmetros da célula unitária.
    unit_angles: Ângulos da célula unitária.

    Retorna:
    HKLS: Lista de índices (hkl).
    Counts: Lista de multiplicidades.
    theta: Lista de ângulos de difração (em graus).
    Gs: Lista de valores G(hkl).
    """
    planes = {}

    indices_range = range(-max_index, max_index + 1)
    ...
    for h in indices_range:
        for k in indices_range:
            for l in indices_range:
                ...
                hkl = (h, k, l)
                G_ = Ghkl(hkl, unit_params, unit_angles)
                G = np.sqrt(np.abs(G_))
                ...
                angle_2theta = abs(np.round(2 * np.degrees(np.arcsin(sintheta)), 2) )
                if angle_2theta < 5:
                    continue

                # Se já existe esse ângulo, atualiza a contagem e mantém o plano "mais simples"
                if angle_2theta in planes:
                    planes[angle_2theta]["count"] += 1
                    existing_plane = planes[angle_2theta]["plane"]
                    if sum(map(abs, hkl)) >= sum(map(abs, existing_plane)):
                        planes[angle_2theta]["plane"] = hkl
                else:
                    planes[angle_2theta] = {
                        "plane": hkl,
                        "count": 1,
                        "theta": angle_2theta / 2,  # armazenar já dividido por 2
                        "Ghkl": G_
                    }

    # Conversão dos dados para arrays e ordenação por ângulo theta
    sorted_angles = sorted(planes.items(), key=lambda x: x[1]["theta"])
    
    HKLS = np.array([item[1]["plane"] for item in sorted_angles])
    Counts = np.array([item[1]["count"] for item in sorted_angles])
    theta = np.array([item[1]["theta"] for item in sorted_angles])
    Gs = np.array([item[1]["Ghkl"] for item in sorted_angles])

    return HKLS, Counts, theta, Gs
...
def FhklBTphkl_NEW(hkls, thetas, Pos, fs, lamb, L, Gamm,BT, counts):
    """
    ...
    Args:
        ...
        fs: atomic form factor in an array with each line being a different atom
        lamb: Wavelength
    ...
    # Convert unit cell parameters into reciprocal vectors
    a_, b_, c_ = a_b_c_(L, Gamm)
    
    # Compute structure factors
    F_all = np.zeros(len(hkls), dtype=complex)
    
    # Precompute atomic scattering factors
    #fs = fhlk2(Z, ai, bi, ci, thetas, lamb)  # Assume this returns an array with fs[i] for each atom
    
    # Compute reciprocal lattice vectors for all hkl indices at once
    G_vectors = np.array([h * a_ + k * b_ + l * c_ for h, k, l in hkls])
    
    # Compute sin^2(theta) / lambda^2 for Debye-Waller factor
    #s2 = (np.sin(np.radians(thetas)) / lamb) ** 2
    
    # Loop over all atoms
    for i, atom_positions in enumerate(Pos):
        #Debye = np.exp(-BT[i] * s2)  # Precompute Debye-Waller factor
        
        # Compute dot products using broadcasting
        phase_factors = np.exp(2j * np.pi * np.dot(atom_positions, G_vectors.T))
        
        # Sum over all symmetry-related positions
        F_all += fs[i] * np.sum(phase_factors, axis=0) #* Debye
...
def simulate_xrd(lamb, max_index, unit_params, unit_angles , positions, 
    ...
    hkls, counts, thetas, Gs = generate_hkls(lamb, max_index, unit_params, unit_angles)
    ...
    intensities = 100 * intensities / np.max(intensities)

   # mask = intensities>=5
   #masks = intensities >=5
    
    return {
        'hkls': hkls[intensities >=5],#[mask],
        'two_thetas': 2*thetas[intensities >=5],#[mask],
        'FsB':F_hkls[intensities >=5],#[mask],
        'intensities': intensities[intensities >=5],#[mask],
        'G': np.sqrt(Gs[intensities >=5]),#[mask]), #d = 1 / Gs
        'multiplicities': counts[intensities >=5],#[mask],
        'Lp':Lp(thetas[intensities >=5]),#[mask]),
        'fhkls':fs,
        'Zs':Zs
    }
